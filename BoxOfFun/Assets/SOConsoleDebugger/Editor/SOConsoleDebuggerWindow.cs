...
public class SOConsoleDebuggerWindow : EditorWindow
{
    /*
    ...
    //Use for getting Unity debug messages
    private List<Log> logs;
    private Log selectedLog;

    [MenuItem("Window/ScriptableObjectConsole")]
    private static void ShowWindow()
    {
        //Show existing window instance. If one does not exist, make one
        EditorWindow.GetWindow(typeof(SOConsoleDebuggerWindow));
    }
    ...
    private void OnEnable()
    {
        ...
        textAreaStyle.normal.background = EditorGUIUtility.Load("builtin skins/darkskin/images/projectbrowsericanareabg.png") as Texture2D;
    ...
    private void OnGUI()
    {
        ...
        if (GUI.changed) Repaint(); //If GUi changed repaint the Window
    }
    ...
    private void DrawMenuBar()
    {
        ...
        {
            ClearLog(); //Clear the log list 
        }

        GUILayout.Space(7);

        collapse = GUILayout.Toggle(collapse, new GUIContent("Collapse"), EditorStyles.toolbarButton, GUILayout.Width(55));
